...
class Pose:
    ...
    def get_bbox(keypoints):
        ...
        for kpt_id in range(Pose.num_kpts):
            if keypoints[kpt_id, 0] == -1:  # 只需要对比第一个元素即可，不要两个都对比
                continue
    ...
    def draw(self, img):
        ...
        # self.keypoints为原图中实际关键点
        for part_id in range(len(BODY_PARTS_PAF_IDS) - 2):  # 为什么是在36里面进行循环，也并没有用到BODY_PARTS_PAF_IDS内的数据。
            kpt_a_id = BODY_PARTS_KPT_IDS[part_id][0]  # 每次只获取第一个元素

            global_kpt_a_id = self.keypoints[kpt_a_id, 0]
            if global_kpt_a_id != -1:
                x_a, y_a = self.keypoints[kpt_a_id]  # 获得关键点坐标

                # cv2.circle：img,圆心(x,y),半径，颜色，-1表示实心圆
